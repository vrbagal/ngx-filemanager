...
export class FileManagerService {
  ...
  setList(path: string) {
    ...
    this.apiService.getList(path).subscribe((data:any)=>{      
      let fm = new FileItem();
      fm.name =this._config.rootPath;
      fm.id = "qwqw";
      fm.path = (path?this._config.rootPath+path:this._config.rootPath);
      fm.subItems = data.result.map(x=> {return{ name:x.name,type:x.type,path:fm.path+'\\'+x.name,subItems:[]}});
      this.rootItem = fm;
    })
